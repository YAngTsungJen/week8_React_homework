...
import { createAsyncDeleteAllCart, createAsyncDeleteSingleCart, createAsyncGetCart, createAsyncUpdateCart } from "../../slices/cartSlice";
...
function Carts(){
    ...
    const updateCart = async(cartId,product_id,qty=1) => {
        ...
        setIsLoading(true);
        try {
            await dispatch(createAsyncUpdateCart({
                cartId,
                product_id,
                qty
            })).unwrap();
            ...
            <div className="container py-5">
                ...
                <div className="bg-light p-4 mt-4 d-flex flex-column align-items-end">
                    <div className="d-flex justify-content-between align-items-center w-100 mb-2" style={{ maxWidth: '300px' }}>
                        ...
                        <span className="fs-5 fw-bold text-end text-nowrap text-danger">NT$ {thousandsStamp(final_total)}</span>
                    </div>
                    <div className="mt-auto mb-3 mt-2">
                        ...
                        <button type="button" className="btn btn-dark btn-lg rounded-0 px-5 mt-3" onClick={handleCheckout} disabled={!carts?.length}>前往結帳</button>
                    </div>
                </div>
                </div>
            ) : (
