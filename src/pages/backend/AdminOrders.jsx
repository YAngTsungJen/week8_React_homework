...
import { getAdminOrdersApi, updateAdminOrderApi, deleteAdminOrderApi,deleteAdminOrdersAllApi } from "../../service/order";
...
function AdminOrders() {
    ...
    return (
        <div className="container py-4">
            <div className="row">
                <div className="col">
                    <div className="sticky-top bg-white py-3" style={{ top: '65px', zIndex: 1020 }}>
                    ...
                    <div className="table-responsive">
                        <table className="table table-hover align-middle">
                            ...
                            <tbody>
                                {isPageLoading ? (
                                    <tr><td colSpan="6" className="text-center py-4"><div className="spinner-border"></div></td></tr>
                                    ...
                                    <tr key={order.id}>
                                        ...
                                        <td>
                                            <div className="btn-group btn-group-sm">
                                                <button className="btn btn-outline-dark" onClick={() => openModal(order)}>查看</button>
                                ...
                                <tr>
                                    <td colSpan="6" className="p-0">
                                        <div className="text-center py-5 border border-dashed">
                                            ...
                                            <Link to="/admin/adminProducts" className="btn btn-outline-dark rounded-0 px-4">
                                                回到產品列表
                                            </Link>
                                        </div>
                                    </td>
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-8">
                                    ...
                                    <table className="table table-sm">
                                        <thead><tr><th>品名</th><th>數量</th><th className="text-end">金額</th></tr></thead>
                                        <tbody>
                                            {tempOrder.products && Object.values(tempOrder.products).map(item => (
                                                <tr key={item.id}>
                                                    <td>{item.product.title}</td>
                                                    <td>{item.qty} / {item.product.unit}</td>
                                                    <td className="text-end">{thousandsStamp(item.final_total)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                    <div className="text-end fw-bold">總計：NT$ {thousandsStamp(tempOrder.total)}</div>
                                </div>
                            </div>
                        </div>
